package com.gotz.custom.calendar
...
import com.gotz.util.CalendarUtil
import com.gotz.util.CalendarUtil.Companion.WEEKS_PER_MONTH
...
class CalendarLongView @JvmOverloads constructor(
    ...
    fun initCalendar(firstDayOfMonth: DateTime, list: List<DateTime>, _listCalendarMemo: List<CalendarMemo>){
        for(idx in 0 until 7){
        ...
        list.forEach{
            val listCalendarMemo = ArrayList<CalendarMemo>()
            val start = it.millis
            val end = it.plusDays(1).millis

            _listCalendarMemo.forEach {
                if(it.start >= start && it.start < end) listCalendarMemo.add(it)
            }
            addView(DayItemLongView(
                context = context,
                date = it,
                firstDayOfMonth = firstDayOfMonth,
                listCalendarMemo = listCalendarMemo
            ))
        }
    }
